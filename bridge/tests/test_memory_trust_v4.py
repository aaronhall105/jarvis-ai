...
async def test_expired_memory_is_not_current_but_history_metadata_remains(tmp_path: Path) -> None:
    ...
    saved = await engine.save(
        "personal",
        "Working location",
        "Aaron is working from home today.",
        owner_key="aaron",
...
async def test_subject_cannot_view_private_revision_history(tmp_path: Path) -> None:
    ...
    saved = await engine.save(
        "personal",
        "Amber surprise",
        "Aaron bought Amber a surprise gift.",
        owner_key="aaron",
        subject_key="amber",
        visibility="private",
    )
    await engine.save(
        "personal",
        "Amber surprise",
        "Aaron changed Amber's surprise gift.",
        owner_key="aaron",
        subject_key="amber",
        visibility="private",
    )
...
async def test_retirement_is_idempotent_and_recoverable(tmp_path: Path) -> None:
    ...
    saved = await engine.save(
        "general",
        "Door code hint",
        "The code hint is the blue book.",
        owner_key="aaron",
        visibility="private",
    )
    ...
    operations = [
        item["operation"] for item in await engine.history(saved["id"], owner_key="aaron")
    ]
