...
class DisconnectAfterTerminalClient(RecordingClient):
    """
...
async def test_first_mobile_text_is_durably_accepted(
    ...
    ledger = RealtimeTurnLedger(tmp_path / "turns.db")
    ...
    try:
        ...
        assert calls == ["Turn the lights off"]

        accepted = [item for item in client.messages if item.get("type") == "turn.accepted"]

        assert len(accepted) == 1
        assert accepted[0]["client_turn_id"] == 41
...
async def test_duplicate_turn_never_runs_brain_twice(
    ...
    try:
        ...
        statuses = [item for item in duplicate.messages if item.get("type") == "turn.status"]

        assert len(statuses) == 1
        assert statuses[0]["found"] is True
        assert statuses[0]["status"] == "completed"
        assert statuses[0]["client_turn_id"] == 52
...
async def test_conflicting_reuse_is_rejected(
    ...
    try:
        ...
        conflicts = [item for item in conflict.messages if item.get("type") == "turn.conflict"]

        assert len(conflicts) == 1
        assert conflicts[0]["client_turn_id"] == 61
...
async def test_turn_status_survives_new_connection_state(
    ...
    reopened = RealtimeTurnLedger(path)
    ...
    async def unused_brain(
        ...
        raise AssertionError("status query must not start a brain turn")

    client = RecordingClient([status_message(77)])
    ...
    try:
        ...
        statuses = [item for item in client.messages if item.get("type") == "turn.status"]
...
async def test_unknown_turn_status_is_explicit(
    ...
    client = RecordingClient([status_message(999)])
...
async def test_legacy_zero_turn_id_keeps_existing_behavior(
    ...
    try:
        ...
        assert calls == ["Legacy request"]

        assert not any(item.get("type") == "turn.accepted" for item in client.messages)

    finally:
        ledger.close()


class TerminalCheckingClient(RecordingClient):
    def __init__(
        self,
        payloads,
        ...
        self._all_expected_terminal = asyncio.Event()
    ...
    async def send_json(
        ...
        event_type = str(payload.get("type") or "")
        turn_id = int(payload.get("client_turn_id") or 0)
        ...
        if expected is not None:
            ...
            if self.checked >= set(self.expectations.keys()):
...
async def test_completed_turn_is_durable_before_terminal_ack(
    ...
    try:
        ...
        assert record.response["text"] == "Completed response."
