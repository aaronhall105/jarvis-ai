...
class HouseContextEngine:
    ...
    def _mentioned(text: str, name: str) -> bool:
        ...
        return bool(words) and all(
            re.search(rf"\b{re.escape(word)}\b", lowered) for word in words[:3]
        )
    ...
    async def context_for(
        ...
        for state in states:
            ...
            display_value = (
                f"{value}{unit}" if unit and value not in {"unknown", "unavailable"} else value
            )
        ...
        for state in states:
            ...
            if old and old[0] != value and sample.observed_monotonic - old[1] <= 900:
                name = self._friendly(state)
