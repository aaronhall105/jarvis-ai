...
class PresenceResolver:
    ...
    async def inspect(self, reference: str) -> dict[str, Any]:
        ...
        person = exact[0]
        raw_attributes = person.get("attributes")
        attributes: dict[str, Any] = raw_attributes if isinstance(raw_attributes, dict) else {}
        ...
        configured_tracker_ids = [
            str(item) for item in tracker_values if str(item).startswith("device_tracker.")
        ]
        related_ids = list(
            dict.fromkeys([*configured_tracker_ids, *([source_id] if source_id else [])])
        )
