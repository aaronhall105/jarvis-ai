...
def _contains_any(text: str, terms: tuple[str, ...]) -> bool:
    ...
    return any(term in words or f" {term} " in f" {text} " for term in terms)
...
def parse_speaker_management_command(
    ...
    if _contains_any(text, ("cancel", "stop", "abort")) and (
        voice_context
        or _contains_any(
            text,
            (
    ...
    if (
        _contains_any(text, relearn_terms) or ("learn" in words and "again" in words)
    ) and voice_context:
        return (
            "relearn",
    ...
    if _contains_any(text, enroll_terms) and (
        voice_context or person_context or ("who" in words and "speaking" in words)
    ):
        return ("enroll", "")
    ...
    if "new" in words and _contains_any(text, ("voice", "speaker", "profile")):
        return ("enroll", "")
    ...
    if _contains_any(text, query_terms) and voice_context and _contains_any(text, knowledge_terms):
        return ("list", "")
...
class SpeakerIdentityClient:
    ...
    def from_environment(cls) -> "SpeakerIdentityClient":
        return cls(
            ...
            base_url=(
                os.getenv("JARVIS_SPEAKER_IDENTITY_URL", DEFAULT_BASE_URL).strip().rstrip("/")
                or DEFAULT_BASE_URL
            ),
        ...
        *,
        ...
        try:
            ...
            message = str(
                ...
                or f"speaker_identity_http_{exc.code}"
            )
    ...
    def finish_enrollment(self, session_id: str) -> dict[str, Any]:
        try:
            return self._request(
                "/enroll/finish", method="POST", payload={"session_id": session_id}
            )
        except SpeakerIdentityError as exc:
            if exc.status == 422 and exc.payload:
                return exc.payload
...
class SpeakerIdentityRuntime:
    ...
    def apply(
        self, metadata: dict[str, Any], result: dict[str, Any], source: str = "voice_id"
    ) -> bool:
    ...
    async def identify(
        self, pcm: bytes, metadata: dict[str, Any], state: dict[str, Any], send: EventSender
    ) -> dict[str, Any] | None:
        if not pcm:
        ...
        recent = state.get("speaker_last_identity")
        if (
            reason in {"too_short", "empty_audio"}
            and isinstance(recent, dict)
            and time.monotonic() - float(recent.get("at", 0)) <= 45
        ):
            cached = recent.get("result")
            if isinstance(cached, dict):
                self.apply(metadata, cached, "recent_short_followup")
    ...
    async def enrollment(
        ...
        if command and command[0] == "cancel":
            ...
            if session_id:
                try:
                    await asyncio.to_thread(self.client.cancel_enrollment, session_id)
                except SpeakerIdentityError:
                    pass
        ...
        if flow.get("phase") == "await_name":
            ...
            try:
                ...
                if exc.status == 409:
                    await speak(
                        f"I already know {name}'s voice. Say, Jarvis, relearn {name}'s voice, if you want to replace it."
                    )
                else:
                    await speak(
                        "I couldn't start voice enrollment. The Voice ID service is unavailable."
                    )
                return True
            phrases = started.get("phrases")
            if not isinstance(phrases, list) or not phrases:
                state.pop("speaker_enrollment", None)
                await speak("I couldn't start a valid voice enrollment session.")
                return True
            flow.update(
                phase="samples",
            ...
            await send(
                {
                    ...
                    "speaker_id": speaker_id,
                    "name": name,
                    "target_samples": started.get("target_samples"),
                }
            )
            await speak(
                f"Great, {name}. I'll learn your voice now. Say this naturally: {phrases[0]}"
            )
            return True
        if flow.get("phase") != "samples":
            state.pop("speaker_enrollment", None)
            return True
        phrases = flow.get("phrases")
        index = int(flow.get("phrase_index", 0))
        if not isinstance(phrases, list) or index >= len(phrases):
            state.pop("speaker_enrollment", None)
            return True
        expected = str(phrases[index])
        if (
            len(normalise_identity_text(transcript).split()) < 4
            or phrase_match_score(transcript, expected) < 0.28
        ):
            await speak(f"Please say the full sentence: {expected}")
            return True
        if not pcm:
            await speak(f"I didn't get a clean voice sample. Please say it again: {expected}")
            return True
        try:
            ...
            await speak(
                "The Voice ID service had a problem with that sample. Please say the sentence again."
            )
            return True
        if not bool(sample.get("accepted")):
            await speak(f"{friendly_quality_message(sample.get('reason'))} {expected}")
            return True
        index += 1
        flow["phrase_index"] = index
        await send(
            {
                "type": "speaker.enrollment.progress",
                "accepted_samples": sample.get("accepted_samples"),
                "target_samples": sample.get("target_samples"),
            }
        )
        if index < len(phrases):
            await speak(f"Good. Next sentence: {phrases[index]}")
            return True
        try:
            finished = await asyncio.to_thread(
                self.client.finish_enrollment, str(flow.get("session_id") or "")
            )
        except SpeakerIdentityError:
            state.pop("speaker_enrollment", None)
            await speak("I couldn't finish the voice profile. Please start voice enrollment again.")
            return True
        if not bool(finished.get("enrolled")):
            state.pop("speaker_enrollment", None)
            await speak(friendly_quality_message(finished.get("reason")))
            return True
        raw_profile = finished.get("speaker")
        profile: dict[str, Any] = raw_profile if isinstance(raw_profile, dict) else {}
        synthetic = {
            "recognized": True,
            "reason": "guided_enrollment",
            "speaker": profile,
            "score": 1.0,
            "margin": 1.0,
        }
        self.apply(metadata, synthetic, "guided_enrollment")
        state["speaker_last_identity"] = {"at": time.monotonic(), "result": synthetic}
        state.pop("speaker_enrollment", None)
        name = str(profile.get("display_name") or "you")
        await send(
            {
                "type": "speaker.enrollment.done",
                "speaker_id": profile.get("speaker_id"),
                "name": name,
    ...
    async def process(
        ...
        if action == "list":
            try:
                data = await asyncio.to_thread(self.client.list_speakers)
            except SpeakerIdentityError:
                await speak("I can't reach the Voice ID service right now.")
                return True
            names = [
            ...
            answer = (
                "I don't have any saved voice profiles yet."
                if not names
                else (
                    f"I recognise {names[0]}."
                    if len(names) == 1
                    else "I recognise " + ", ".join(names[:-1]) + f" and {names[-1]}."
                )
            )
            await speak(answer)
            return True
        if action == "enroll":
            ...
            await speak("Of course. What is the person's name?")
            return True
        current_id = str(metadata.get("speaker_id") or "")
        current_name = str(metadata.get("speaker_name") or "")
        admin = bool(metadata.get("speaker_household_admin"))
        if action == "relearn":
            requested = name or (current_name if current_id not in {"", "unknown"} else "")
            requested_id = speaker_id_from_name(requested)
            if not requested_id:
                await speak("Tell me whose voice you want me to relearn.")
                return True
            if not (admin or current_id == requested_id):
                await speak("I can only replace your own voice profile, unless Aaron is speaking.")
                return True
        ...
        if action == "forget":
            ...
            if not admin:
                await speak("Only Aaron can delete household voice profiles.")
                return True
            if not requested_id:
                await speak("Tell me whose voice profile to forget.")
                return True
            try:
                deleted = await asyncio.to_thread(self.client.delete_speaker, requested_id)
            except SpeakerIdentityError:
                await speak("I couldn't find that saved voice profile.")
                return True
            await speak(f"I've forgotten {deleted.get('display_name') or name}'s voice profile.")
            return True
        return False
