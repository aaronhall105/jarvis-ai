...
/** Fully local, account-free Jarvis keyword detector powered by sherpa-onnx. */
final class SherpaWakeWordEngine {
    ...
    private static final String ASSET_DIR = "sherpa-kws-v1831";
    ...
    SherpaWakeWordEngine(Context context, Listener listener) {
        this.context = context.getApplicationContext();
        this.listener = listener;
    }

    synchronized void start(float sensitivity) {
        stop();
        running = true;
        worker = new Thread(
            () -> runDetector(sensitivity),
            "jarvis-sherpa-wake"
        );
        worker.start();
    }
    ...
    private void runDetector(float sensitivity) {
        ...
        try {
            ...
            if (minimum <= 0) {
                throw new IllegalStateException("Android returned an invalid microphone buffer");
            }
            ...
            localRecorder = new AudioRecord.Builder()
                ...
                .setBufferSizeInBytes(Math.max(minimum * 2, 6_400))
                .build();

            if (localRecorder.getState() != AudioRecord.STATE_INITIALIZED) {
                throw new IllegalStateException("Dedicated wake microphone could not initialise");
            }

            recorder = localRecorder;
            localRecorder.startRecording();
            status("Dedicated offline wake word active — say \"Jarvis\"");
            ...
            while (running && !Thread.currentThread().isInterrupted()) {
                ...
                while (running && spotter.isReady(stream)) {
                    ...
                    String keyword =
                        result == null || result.getKeyword() == null
                            ? ""
                            : result.getKeyword().trim();
            ...
            if (localRecorder != null) {
                try { localRecorder.stop(); } catch (Exception ignored) {}
                try { localRecorder.release(); } catch (Exception ignored) {}
            }
            if (stream != null) {
    ...
    private File installModelAssets() throws Exception {
        File modelDir = new File(context.getNoBackupFilesDir(), ASSET_DIR);
        if (!modelDir.exists() && !modelDir.mkdirs()) {
            throw new IllegalStateException("Could not create wake-word model directory");
        }

        for (String name : MODEL_FILES) {
            File output = new File(modelDir, name);
            if (output.isFile() && output.length() > 0L) continue;

            File temporary = new File(modelDir, name + ".tmp");
            try (
                InputStream input = context.getAssets().open(ASSET_DIR + "/" + name);
                FileOutputStream destination = new FileOutputStream(temporary)
            ) {
