...
public final class JarvisRealtimeClient {
    public interface Listener {
        ...
        void onAudioDone();
        void onToolStarted(String command);
    ...
    public boolean sendAudio(byte[] pcm16) {
        ...
        // Do not allow stale microphone audio to build up on a poor connection.
        if (current.queueSize() > 512_000L) return false;
    ...
    public void sendText(String text) {
        WebSocket current = socket;
        if (!authenticated || current == null || text == null || text.isBlank()) return;
        try {
            current.send(RealtimeProtocol.text(text.trim()));
    ...
    private void open() {
        ...
        socket = http.newWebSocket(request, new WebSocketListener() {
            @Override public void onOpen(WebSocket webSocket, Response response) {
                ...
                try {
                    webSocket.send(RealtimeProtocol.auth(token, deviceId, userName));
                } catch (Exception exception) {
    ...
    private void handleText(String raw) {
        ...
        switch (event.type) {
            ...
            case "ready" -> post(() -> listener.onReady(event.model));
            ...
            case "tool.started" -> post(() -> listener.onToolStarted(
                event.command.isBlank() ? (event.text.isBlank() ? event.message : event.text) : event.command
            ));
            case "error" -> post(() -> listener.onError(event.message.isBlank() ? "Realtime voice error" : event.message));
            default -> { }
        }
    }
    ...
    private void scheduleReconnect() {
        ...
        long delay = Math.min(10_000L, 750L * (1L << attempt));
        main.postDelayed(() -> {
