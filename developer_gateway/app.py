    ...
    try:
        ...
        async def emit(message: dict[str, Any]) -> None:
            ...
            if event_thread and event_thread not in owned_threads:
                return
            if message.get("id") is not None and str(message.get("method", "")).endswith(
                "requestApproval"
            ):
        ...
        await socket.send_json(
            {
                ...
                "workspaces": [
                    {
                        "id": key,
                        "name": "Jarvis Wear" if key == "jarvis-wear" else "Jarvis",
                        "path": str(path),
                    }
        ...
        while True:
            ...
            if kind == "threads.list":
                ...
                result = result_or_raise(
                    await codex.request(
                        "thread/list", {"cwd": str(path), "limit": 50, "sortKey": "updated_at"}
                    )
                )
                ...
                result = result_or_raise(
                    await codex.request(
                        "thread/name/set",
                        {"threadId": str(message["thread_id"]), "name": str(message["name"])[:120]},
                    )
                )
