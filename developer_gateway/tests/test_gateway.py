...
def test_authenticated_client_can_delete_only_resumed_thread(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr("developer_gateway.app.TOKEN", "correct-token")
    monkeypatch.setattr("developer_gateway.app.codex", FakeCodex())
...
def test_attachment_content_is_bounded_and_never_accepts_paths(
...
def test_image_attachment_is_copied_to_private_gateway_storage(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import base64
    ...
    inputs = codex_inputs(
        "inspect",
        [{"name": "screen.png", "mime": "image/png", "data": base64.b64encode(b"png").decode()}],
    )
