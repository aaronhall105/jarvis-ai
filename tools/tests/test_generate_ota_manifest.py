...
def test_manifest_uses_exact_artifact(tmp_path: Path) -> None:
    ...
    result = MODULE.generate(
        apk, "19.0.0-alpha15", 190150, "a" * 40, "Release notes", "2026-08-20T12:00:00Z"
    )
...
@pytest.mark.parametrize(
    "version,expected",
    [("19.0.0-alpha16", "alpha"), ("19.0.0-beta1", "beta"), ("19.0.0", "stable")],
)
def test_channel(version: str, expected: str) -> None:
    assert MODULE.channel(version) == expected
...
def test_invalid_inputs_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError):
        MODULE.generate(apk, "19.0.0-alpha15", 190150, "a" * 40, "notes", "2026-08-20T12:00:00Z")
    with pytest.raises(ValueError):
        MODULE.channel("19.0.0-nightly1")
