...
def generate(apk: Path, version: str, version_code: int, commit: str, notes: str, published: str) -> dict[str, object]:
...
def main() -> None:
    ...
    parser.add_argument("--apk", type=Path, required=True); parser.add_argument("--version", required=True)
    parser.add_argument("--version-code", type=int, required=True); parser.add_argument("--commit", required=True)
    parser.add_argument("--notes-file", type=Path, required=True); parser.add_argument("--published-at")
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    published = args.published_at or datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    result = generate(args.apk, args.version, args.version_code, args.commit, args.notes_file.read_text(), published)
